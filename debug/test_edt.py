...
for i in range(1,49):
    ...
    x.astype(np.float32).tofile("slice_0063.f32") 
    rel_eb = 0.005
    abs_eb = rel_eb * (x.max() - x.min()) 
    edt_result = edt_model("slice_0063.f32", rel_eb, x.shape) 
    os.remove("slice_0063.f32")
    edt_psnr = get_psnr(x, edt_result)
    edt_ssim = qcatssim(x, edt_result)
    ...
    print(f"Time step {i}: pred psnr: {pred_psnr}, pred ssim: {pred_ssim}") 
