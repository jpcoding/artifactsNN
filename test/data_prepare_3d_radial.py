...
# import baseline from debug.baseline
sys.path.append(os.path.abspath(os.path.join(os.path.dirname(__file__), '../debug'))) 
import baseline 
...
# decompressed, quant_idx = quantization(orig_data, precision)
sz3_path = "/lcrc/project/ECP-EZ/jp/git/SZ3/build_lorenzo/tools/sz3/sz3"
decompressed, cr= baseline.sz3(orig_data, rel_eb, orig_data.shape, 
                            '/lcrc/project/ECP-EZ/jp/git/arcnn/debug/configs/sz3_interp.config',
                            compressor_path=sz3_path) 

print("psnr = ", get_psnr(orig_data, decompressed))
print("ssim = ", stats.qcatssim(orig_data, decompressed)) 
print("cr = ", cr) 
...
subsubfolders = ['decompressed', 'orig_data'] 
...
def process_block(x1, y1, z1, block_size, shape, quant_idx, 
                  decompressed, orig_data, numpy_data_type, subsubfolder_path):
    ...
    block_quant = decompressed[x1:x2, y1:y2, z1:z2]
    block_orig = orig_data[x1:x2, y1:y2, z1:z2]

    entropy_val = entropy(block_qidx)

    decompressed_path = os.path.join(subfolder_path, f'decompressed/data_{x1}_{y1}_{z1}_{entropy_val:.3f}.f32')
    orig_data_path = os.path.join(subfolder_path, f'orig_data/data_{x1}_{y1}_{z1}_{entropy_val:.3f}.f32')
    print(f"Saving quantized data to {decompressed_path}")

    block_quant.astype(numpy_data_type).tofile(decompressed_path)
...
with ProcessPoolExecutor() as executor:
    for i, j, k in tasks:
        process_block(i, j, k, block_size, shape, decompressed, 
                      decompressed, orig_data, 
